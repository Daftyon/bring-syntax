...
function activate(context) {
    ...
    const symbolProvider = vscode.languages.registerDocumentSymbolProvider({ language: 'bring' }, new BringDocumentSymbolProvider());
    // Register completion provider
    const completionProvider = vscode.languages.registerCompletionItemProvider({ language: 'bring' }, new BringCompletionProvider(), '=', '@', '{', '[');
    // Register hover provider
    const hoverProvider = vscode.languages.registerHoverProvider({ language: 'bring' }, new BringHoverProvider());
    // Add to subscriptions
    context.subscriptions.push(validateCommand, formatCommand, symbolProvider, completionProvider, hoverProvider);
    // Show welcome message
    vscode.window.showInformationMessage('🚀 Bring Language Support loaded!');
}
exports.activate = activate;
function validateBringFile(document) {
    const diagnostics = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if ((line.includes('"') && line.split('"').length % 2 === 0) ||
            ...
            const diagnostic = new vscode.Diagnostic(new vscode.Range(i, 0, i, line.length), 'Unterminated string', vscode.DiagnosticSeverity.Error);
        ...
        if (attrMatch) {
            attrMatch.forEach(attr => {
                if (!attr.match(/@\w+=(true|false|\d+(\.\d+)?|"[^"]*"|'[^']*')/)) {
                    ...
                    const diagnostic = new vscode.Diagnostic(new vscode.Range(i, startPos, i, startPos + attr.length), 'Invalid attribute syntax', vscode.DiagnosticSeverity.Warning);
                    diagnostics.push(diagnostic);
                }
            });
        }
...
function formatBringDocument(editor) {
...
class BringDocumentSymbolProvider {
    provideDocumentSymbols(document) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (schemaMatch) {
                const symbol = new vscode.DocumentSymbol(schemaMatch[1], 'Schema', vscode.SymbolKind.Class, new vscode.Range(i, 0, i, line.length), new vscode.Range(i, 0, i, line.length));
                symbols.push(symbol);
            }
            // Key-value pairs
            const kvMatch = line.match(/^(\w+)\s*(?:@[^=]*=[^@\s]*\s*)*=\s*/);
            if (kvMatch) {
                const symbol = new vscode.DocumentSymbol(kvMatch[1], 'Property', vscode.SymbolKind.Property, new vscode.Range(i, 0, i, line.length), new vscode.Range(i, 0, i, line.length));
                symbols.push(symbol);
            }
        }
        return symbols;
...
\`\`\`
        ...
        if (line.includes(`@${word}=`)) {
            ...
            if (attributeHelp) {
                return new vscode.Hover(new vscode.MarkdownString(attributeHelp), range);
            }
        }
        // Hover for types - FIX: Use proper typing
        const typeHelp = this.getTypeHelp(word);
        if (typeHelp) {
            return new vscode.Hover(new vscode.MarkdownString(`**${word}** - ${typeHelp}`), range);
        }
        return undefined;
    }
    getTypeHelp(type) {
